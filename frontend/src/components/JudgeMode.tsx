...
import { Network, X, CheckCircle2, Gauge, Target, Loader2, Bot, Sparkles } from 'lucide-react';
...
import { AIFaultInjector } from './judge/AIFaultInjector';
  ...
  return (
    <div className="w-full h-full bg-background text-textMain flex flex-col overflow-hidden">
      ...
      <header className="h-12 glass-header px-6 flex items-center justify-between shrink-0 border-b border-border">
        ...
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1.5 text-[10px] font-mono text-primary bg-primary/10 border border-primary/20 px-2.5 py-1 rounded">
        ...
        <div className="w-[400px] flex flex-col gap-4 overflow-y-auto">
          ...
          <div className="glass-panel p-1 flex gap-1">
            <button
              ...
              className={clsx(
                'flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-mono font-bold uppercase rounded-lg transition-colors',
                rightTab === 'ai' ? 'bg-primary text-white' : 'text-textMuted hover:text-textMain hover:bg-surfaceHover'
              )}
            >
            ...
            <button
              ...
              className={clsx(
                'flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-mono font-bold uppercase rounded-lg transition-colors',
                rightTab === 'manual' ? 'bg-primary text-white' : 'text-textMuted hover:text-textMain hover:bg-surfaceHover'
              )}
            >
          ...
          {rightTab === 'ai' && (
            <>
              ...
              <div className="glass-panel p-5">
                <AIFaultInjector
                  onInjected={() => {
                    // Reset seed for next run
                    setSeed(`judge-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`);
                  }}
                />
              </div>
              ...
              <div className="glass-panel p-4">
                <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.2em] text-textMuted mb-2">
                  <span>Current tick</span>
                  <span className="font-mono text-textMain">{bundle.playback.tick}</span>
                </div>
                <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.2em] text-textMuted mb-2">
                  <span>Top hypothesis</span>
                  <span className="font-mono text-textMain">{currentTopCandidate ?? 'awaiting…'}</span>
                </div>
                ...
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-surface border border-border">
                  <div className="h-full rounded-full bg-primary transition-all duration-500" style={{ width: `${Math.max(4, confidencePct)}%` }} />
                </div>
              </div>

              {/* Analyst Panel (compact) */}
              <div className="glass-panel p-5">
                <AnalystPanel compact />
              </div>
          ...
          {rightTab === 'manual' && (
            <>
              ...
              <TopHypothesis />
              {bundle.prediction && <PredictionCard prediction={bundle.prediction} />}
              {(bundle.status === 'VALIDATED' || bundle.status === 'COMPLETED') && <ValidationResult />}
              ...
              {groundTruth && (
                <div className="glass-panel border border-border p-4">
                  ...
                  <div className={clsx(
                    ...
                    {currentTopCandidate ?? 'N/A'} vs {groundTruth.trueRootService}/{groundTruth.trueRootResource} — {currentTopCandidate === `${groundTruth.trueRootService}/${groundTruth.trueRootResource}` ? 'MATCH' : 'MISMATCH'}
                  </div>
                  <button
                    onClick={revealGroundTruth}
